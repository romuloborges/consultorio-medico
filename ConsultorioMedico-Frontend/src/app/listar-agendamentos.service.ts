...
export class ListarAgendamentoService {
    ...
    obterAgendamentosDataAtual() {
        ...
        // });
        this.httpClient
  .get<Scheduling[]>(`${applicationUrl}/agendamento/2019-12-28`)
  .pipe(
    map((sch: any) => {
      return new Scheduling(
        sch.idAgendamento,
        new Date(this.datePipe.transform(sch.dataHoraAgendamento, this.FORMAT)),
        new Date(this.datePipe.transform(sch.dataHoraRegistro, this.FORMAT)),
        new Person(
        ...
        new Doctor(sch.medicoListarViewModel.idMedico, sch.medicoListarViewModel.nomeMedico),
